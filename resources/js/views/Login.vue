        ...
        <div class="w-full max-w-[480px] z-10">
            <div class="bg-surface-container p-6 sm:p-10 md:p-14 rounded-2xl border border-outline-variant shadow-[0_8px_30px_rgba(0,0,0,0.45)] flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div class="text-center space-y-2">
                    <h1 class="font-serif text-3xl font-bold text-primary">Welcome Back</h1>
                    <p class="text-sm text-on-surface-variant">Sign in to your private cinema sanctuary.</p>
                </div>
                ...
                <div class="text-center">
                    <p class="text-sm text-on-surface-variant">
                        ...
                        <router-link to="/register" class="text-primary font-bold hover:underline underline-offset-4 ml-1">Join Cinehall</router-link>
                    </p>
                </div>
            </div>

            <!-- Quote/Feature under card -->
            <div class="mt-12 text-center max-w-sm mx-auto opacity-80">
                <p class="text-xs italic text-outline leading-relaxed">
                    "The details are not the details. They make the design."
                </p>
                <p class="text-[9px] text-outline-variant mt-2 uppercase tracking-widest font-bold">— The Cinema Aesthetic</p>
            </div>
        </div>
    </main>
</template>
...
const handleLogin = async () => {
    ...
    try {
        ...
        error.value = err.response?.data?.message || 'Authentication rejected. Check specified identifiers.';
    } finally {
        loading.value = false;
    }
};
